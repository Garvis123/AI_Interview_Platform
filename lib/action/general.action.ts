...
export async function createFeedback(params: CreateFeedbackParams) {
  ...
  try {
    ...
    const { object } = await generateObject({
      ...
      system:
        "You are a professional interviewer analyzing a mock interview. Evaluate the candidate based only on the provided categories.",
      prompt: `
...
export async function getLatestInterviews(
  ...
  try {
    const interviews = await db
      ...
      // ❗ Firestore doesn’t support `!=` without indexes. Safer to filter client-side.
      .limit(limit)
      .get();

    return interviews.docs
      .map((doc) => ({ id: doc.id, ...doc.data() } as Interview))
      .filter((i) => i.userId !== userId); // exclude current user on client-side
  } catch (error) {
